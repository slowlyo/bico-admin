...
// 辅助函数：从对象中提取总数
function extractTotal(obj: Record<string, unknown>, list: unknown[], fields: string[]): number {
  for (const field of fields) {
...
function extractPagination(
...
): Pick<ApiResponse<unknown>, 'page' | 'pageSize'> | undefined {
  const result: Partial<Pick<ApiResponse<unknown>, 'page' | 'pageSize'>> = {}
  ...
  for (const src of sources) {
    for (const field of currentFields) {
      if (field in src && typeof src[field] === 'number') {
        result.page = src[field] as number
        break
      }
    }
    if (result.page !== undefined) break
  }
  ...
  if (result.page === undefined && result.pageSize === undefined) return undefined
  return result as Pick<ApiResponse<unknown>, 'page' | 'pageSize'>
}
...
export const defaultResponseAdapter = <T>(response: unknown): ApiResponse<T> => {
  ...
  let list: T[] = []
  let total = 0
  let pagination: Pick<ApiResponse<unknown>, 'page' | 'pageSize'> | undefined

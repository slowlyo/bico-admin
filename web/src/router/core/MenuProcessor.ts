...
export class MenuProcessor {
  ...
  private async processFrontendMenu(): Promise<AppRouteRecord[]> {
    ...
    // 根据权限过滤菜单
    menuList = this.filterMenuByPermissions(menuList, permissions)
  ...
  private filterMenuByPermissions(
    ...
    return menu.reduce((acc: AppRouteRecord[], item) => {
      ...
      if (!itemPermissions || (Array.isArray(itemPermissions) && itemPermissions.length === 0)) {
        ...
        // 2. 校验具体权限标识
        const hasPermission = Array.isArray(itemPermissions)
          ? itemPermissions.some((p) => permissions.includes(p))
          : permissions.includes(itemPermissions as string)
  ...
  private filterEmptyMenus(menuList: AppRouteRecord[]): AppRouteRecord[] {
    return menuList
      ...
      .filter((item) => {
        // 如果配置了 hideInMenu，直接过滤掉
        if (item.meta?.hideInMenu === true) {
          return false
        }

        // 如果定义了 children 属性（即使是空数组且没有被过滤掉），说明这是一个目录菜单，应该保留
        if (item.children && item.children.length >= 0) {
          return true
        }

...
function useTableImpl<TApiFn extends (params: any) => Promise<any>>(
  ...
  // 缓存清理定时器
  let cacheCleanupTimer: any = null
   ...
   * 处理排序变化
   */
  const handleSortChange = async ({ prop, order }: { prop: string; order: string }): Promise<void> => {
    const paramsRecord = searchParams as Record<string, unknown>
    paramsRecord.sortField = prop
    // 将 Element Plus 的排序值转换为后端预期的值
    paramsRecord.sortOrder = order === 'ascending' ? 'ascend' : order === 'descending' ? 'descend' : ''
    pagination.current = 1
    paramsRecord[pageKey] = 1

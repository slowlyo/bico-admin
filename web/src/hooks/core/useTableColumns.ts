...
export function useTableColumns<T = any>(
  ...
  // 初始列配置过滤权限
  const filterAuthColumns = (cols: ColumnOption<T>[]) => {
    return cols.filter((col) => {
      if (!col.auth) return true
      if (Array.isArray(col.auth)) {
        return col.auth.some((authMark) => hasAuth(authMark))
      }
      return hasAuth(col.auth)
    })
  }

  const dynamicColumns = ref<ColumnOption<T>[]>(filterAuthColumns(columnsFactory()))
  ...
  const setDynamicColumns = (updater: (cols: ColumnOption<T>[]) => void | ColumnOption<T>[]) => {
    ...
    dynamicColumns.value = Array.isArray(result) ? filterAuthColumns(result) : filterAuthColumns(copy)
  }
  ...
  return {
     ...
     */
    resetColumns: () => {
      dynamicColumns.value = filterAuthColumns(columnsFactory())
    },
